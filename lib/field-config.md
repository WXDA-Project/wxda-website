...
Single source of truth for every field that appears in the WXDA document search, results table, and record detail page. Editing this file is the **only** change needed to add, remove, rename, or reconfigure a field — nothing else in the codebase needs to change.
...
Downstream code imports the pre-derived constants (e.g. `TABLE_FIELDS`, `SORT_DATE_KEY`) rather than the raw array, so there are no hardcoded column names anywhere outside this file.
...
|---|---|---|---|
| `key` | `string` | yes | Column name in the `documents` Supabase table. This is the only value that must match the database. |
...
| `isArray` | `boolean` | no | True when the database column is a `text[]` array. Affects how multiselect filters are applied (`overlaps` vs `eq`) and how values are rendered. |
...
|---|---|---|
| `'primary-date'` | The main date column — used for ORDER BY and date-range filtering | `SORT_DATE_KEY`, `DATE_FILTER_FIELD` |
| `'location'` | The array of location mentions — used by the map feature | `LOCATION_FIELD_KEY` |
...
| `'person-sort'` | (persons only) Default ORDER BY column for person search | `PERSON_SORT_KEY` |
| `'person-badge'` | (persons only) Displayed as a badge chip on the person detail page | `PERSON_BADGE_FIELDS`, `PERSON_BADGE_KEYS` |

Only one field should carry each role. The derived constants use `Array.find()` / `Array.filter()` and will throw at startup (`!` assertion) if the role is missing.
...
|---|---|---|---|
| `UNIQUE_FIELDS` | `FieldConfig[]` | queries.ts | De-duplicated field list (a field declared in both a filter section and a detail section only appears once). |
| `TABLE_FIELDS` | `FieldConfig[]` | search/page.tsx | Fields with `showInTable: true`, in declaration order → table columns. |
| `DETAIL_FIELDS` | `FieldConfig[]` | record/[id]/page.tsx | Fields with `showInDetail: true`, in declaration order → detail key-value list. |
| `FILTER_FIELDS` | `FieldConfig[]` | SearchFilters.tsx | Fields with a `filterType` set → all filter controls. |
| `MULTISELECT_FILTER_FIELDS` | `FieldConfig[]` | queries.ts, search/page.tsx | Subset of `FILTER_FIELDS` with `filterType === 'multiselect'` → checkbox filters. |
| `FIELD_MAP` | `Record<string, FieldConfig>` | — | Key → config lookup for direct field access by column name. |
| `ENRICHED_KEYS` | `Set<string>` | record/[id]/page.tsx | Keys of fields with `enriched: true` → skipped in the generic detail list. |
| `DOC_SUMMARY_COLUMNS` | `string` | queries.ts | Comma-separated SELECT string of `id` + all `showInDocSummary: true` fields. Used when fetching a person's associated documents. |
| `DATE_FILTER_FIELD` | `FieldConfig` | SearchFilters.tsx, TimelineChart.tsx | Full config entry for the `'primary-date'` role field. |
| `SORT_DATE_KEY` | `string` | queries.ts | Column key for ORDER BY and date-range queries. |
| `LOCATION_FIELD_KEY` | `string` | queries.ts | Column key for the location array (map feature). |
| `AUTHOR_FIELD_KEY` | `string` | queries.ts, record/[id]/page.tsx | Column key for the author FK array. |
| `CONTAINER_FIELD_KEY` | `string` | queries.ts, record/[id]/page.tsx | Column key for the container FK. |
| `CITE_AS_KEY` | `string` | record/[id]/page.tsx | Column key for the cite-as string. |
| `SOURCE_URL_KEY` | `string` | record/[id]/page.tsx | Column key for the source URL. |
| `FTS_COLUMN` | `'fts'` | queries.ts | Name of the tsvector full-text search column. Not a user-facing field — defined here to keep all column references in one place. |
| `VISIBILITY_COLUMN` | `'visibility'` | queries.ts | Row-visibility gate column. Only rows with value `'public'` are served. |
...
That's it. The filter appears automatically in the sidebar, the URL parameter is handled, the query applies `overlaps()` (array) or `in()` (scalar), and distinct options are fetched from the database at runtime. No changes needed in `queries.ts`, `SearchFilters.tsx`, or the search page.
...
```ts
{
  ...
  minDate: '1750-01-01',  // ← change here
  maxDate: '1900-12-31',  // ← and here
  ...
}
```
...
If a new field needs to be referenced by a stable constant elsewhere in the code, add a role and export a constant at the bottom of the file:

```ts
// In FIELDS:
{ key: 'my_special_col', role: 'my-role', ... }

// At the bottom of field-config.ts:
...
They are defined here rather than inline in `queries.ts` so that renaming them is still a single-file change.
